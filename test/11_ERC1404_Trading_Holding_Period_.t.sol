...
contract ERC1404_Trading_Holding_Period is ERC1404_Base_Setup {
    ...
    function testTradingHoldingPeriodIssuerCanTransfer(
        uint amount
    ) public {
    ...
    function testTradingHoldingPeriodInvestorCannotTransfer(
        uint amount
    ) public {
        amount = bound(amount, 1, initialSupply);
        token.transfer(addr1, amount);
    ...
    function testTradingHoldingPeriodinvestorsCanTransferToEachother(
        uint amount
    ) public {
        amount = bound(amount, 1, initialSupply);
    ...
    function testTradingHoldingPeriodToZeroinvestorsCanTransferToEachother(
        uint amount
    ) public {
        amount = bound(amount, 1, initialSupply);
        // Add balance to investor
